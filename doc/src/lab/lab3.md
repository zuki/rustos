...
思います。_カレントワーキングディレクトリ_ (`cwd`)とはそこを起点に
相対的ファイルアクセスが行われるディレクトリのことです。たとえば、
`cwd`が`/a`の場合、`hello`にアクセスするとファイル`/a/hello`に
アクセスすることになります。`cwd`が`/a/b/c`の場合は`hello`に
アクセスすると`/a/b/c/hello`にアクセスすることになります。任意の
パスの前に`/`を付けることでカレントワーキングディレクトリからの
相対パスではなく、絶対パスにすることができます。そのため、`/hello`は
カレントワーキングディレクトリに関係なく、常にルートディレクトリに
ある`hello`という名前のファイルを参照することになります。

シェルでは`cd <dir>`コマンドを使ってカレントワーキングディレクトリを
`dir`に変更することができます。たとえば、`cd /hello/there`を実行する
と`cwd`は`/hello/there`に変更されます。この後に`cd you`を実行すると
`cwd`は`/hello/there/you`になります。

ほとんどのOSはプロセスのワーキングディレクトリを変更するシステム
コールを提供しています。私達のOSにはまだプロセスもシステムコールも
ないのでシェルで直接`cwd`を管理することになります。

### コマンド

オペレーティングシステムの主要なインターフェイスであるシェルを通して
ファイルシステムを公開する4つのコマンドを実装します。`cd`, `ls`,
`pwd`, `cat`です。この課題ではこれらのコマンドの仕様は次のように
します。

* `pwd` - **ワーキングディレクトリを表示する**

  カレントワーキングディレクトリの完全パスを表示する。

* `cd <directory>` - **（ワーキング）ディレクトリを変更する**

  カレントワーキングディレクトリを`directory`に変更する。
  `directory`引数は必須である。

* `ls [-a] [directory]` - **ディレクトリのファイルを一覧表示する**

  ディレクトリのエントリを一覧表示する。`-a`も`directory`も
  オプションの引数である。`-a`が渡された場合は隠しファイルは
  表示される。それ以外の場合は隠しファイルは表示されない。
  `directory`が渡されなかった場合はカレントワーキングディレクトリの
  エントリが表示される。そうでない場合は`directory`のエントリが
  表示される。引数は一緒に使用できるが`-a`は`directory`より前に
  与えられなければならない。

  無効な引数はエラーとなる。また、`directory`が有効な既存の
  ディレクトリに対応していない場合もエラーとなる。

* `cat <path..>` - **ファイルを連結する**

  指定されたパスにあるファイルの内容を次々に表示する。少なくとも
  1つの`path`引数が必要である。

  `path`が有効な既存のファイルを指していない場合はエラーとなる。
  有効なファイルが無効なUTF-8を含んでいる場合もエラーとなる。

絶対パスでない場合はすべてカレントワーキングディレクトリからの
相対パスとして扱う必要があります。これらのコマンドの実際の動作に
ついては上のGIFを参照してください。これらのコマンドを実装する場合、
すべての情報が存在する限り、ディレクトリエントリとエラーをどのように
表示するかは自由です。

### 実装

`kern/src/shell.rs`にあるシェルを4つのコマンドで拡張してください。
カレントワーキングディレクトリを保持するには可変な[PathBuf](https://doc.rust-lang.org/std/path/struct.PathBuf.html)を
使ってください。この`PathBuf`は`cd`コマンドで変更する必要があります。
各コマンドに共通のシグネチャを持つ関数を作成すると便利でしょう。
型安全性をさらに高めるために実行可能コマンドの概念を抽象化して
各コマンドで実装するトレイトにすることもできます。

上記の仕様に即して4つのコマンドを実装し、テストし、検証したら
課題を提出する準備ができたことになります。おめでとう。

**警告: ビンアロケータを使っていることを確認してください**
> あなたのファイルシステムはおそらく非常にメモリを消費します。
> メモリ不足にならないようにビンアロケータを使用していることを
> 確認してください。

**ヒント**
> PathBufとPathの既存のメソッドを上手に使いましょう。

**ヒント**
> `cd`では`..`と`.`を特別に扱う必要があります。
