...
このサブフェーズではシステムコールライブラリを拡張し、ユーザプログラムに
とってより便利なものにします。また、私たちのOSで実行されるユーザプログラムの
サンプルをRustで書いてコンパイルします。最後に、新しいシステムコールを追加する
ためにシステムコールハンドラを変更します。主に`lib/kernel_api/src/syscall.rs`と
`kern/src/traps/syscall.rs`で作業します。

#### ユーザプログラム

まず、OS用のカーネルAPIライブラリを構築します。このライブラリはOSにシステム
コールをするためにユーザレベルのプログラムからインポートされます。

`lib/kernel_api/src/syscall.rs`の関数を実装することから始めます。各関数は
関数名に対応するシステムコールを呼び出すように実装する必要があります。

例をあげるために私たちのシステムでシステムコールがどのように処理されるかを
見てみましょう。ユーザプログラムはライブラリ`kernel_api`をインポートして
その関数を使用します。`kernel_api`の関数は適切なシステムコール番号と引数を
指定してOSにシステムコールを発行するために必要なアセンブリを実装します。
これはOSの`handle_exception`で捕捉され、`handle_syscall`にリクエストが転送
されます。同様に`handle_syscall`はシステムコール番号に基づいてリクエストを
分類してその担当のハンドラに転送します。ハンドラはシステムコールに必要な
機能を実行して呼び出しの連鎖を通してユーザレベルに復帰します。

`lib/kernel_api/src/syscall.rs`にあるライブラリ関数の実装から開始してください。
これらの関数でしなければならないことは必要なシステムコールの引数を準備して
`svc`命令を発行することだけです。例として`sleep`を用意しました。システム
コール番号は`lib/kernel_api/src/lib.rs`で定義されています。`lib/kernel_api/src/lib.rs`
を参照してシステムコール番号の定義済み変数を探してください。

次に、実装した各システムコールについて、上で説明した処理の連鎖を完成させて
ください。つまり、`handle_syscall`に追加し、`kern/src/traps/syscall.rs`に
それぞれの`sys_{SYSCALL}`関数を作成します。

`write`システムコールの実装から始めることを勧めます。実装が終わったら
`sleep`プログラムと同じ方法で複数の`fib`プログラムを実行させることができます。

すべてがうまくいけば、fibプロセスが実行され、その結果が出力されるはずです。

`kernel_api`を変更するたびにユーザプログラムを再コンパイルする必要があることに
注意してください。

`kernel_api`ユーザライブラリのすべての関数とカーネルのシステムコールハンドラの
実装が終わったら、`fib`プログラムですべてのシステムコールを呼び出してテストして
みてください。また、冒険心が湧いたら、自分のプログラムを書いてみてください。
`fib`のビルドシステムをコピーし、それをあなたのプログラムで使って動作する
バイナリを生成してください。

**警告: 投稿前にコードをチェックしてください**
> コードを投稿する前に`kern/src/param.rs`の`TICK`変数を10ミリ秒に変更して
> ください。さらに、デバッグに使用したシステムコールや例外が残っていないか
> 確認してください。`main`は6行のハズです。5つの`initialize()`の呼び出しと
> 1つの`start()`の呼び出しです。
