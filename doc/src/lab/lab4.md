...
アドレスも同時に実行されているかもしれない他のプロセスの仮想空間を意識
することはありません。
...
断片的な物理メモリページに変換することができます。つまり、物理空間の
断片的なページを利用して、1つの連続した仮想空間を形成することができます。
...
`TTBR1_EL1`の2つあります。私たちの設計では`TTBR1_EL1`は_ユーザページテーブル_
...
> どうやって知るのでしょうか。
    ...
    ヒント: カーネルレベルとユーザーレベルの仮想空間は任意のアドレスで
    分割したわけではありません。カーネルレベルを仮想空間の上位部分に
    したらどうなるでしょうか。次のリファレンスが参考になるでしょう:
    [ARMv8-Aアドレス変換](https://static.docs.arm.com/100940/0100/armv8_a_address%20translation_100940_0100_en.pdf)
