...
- **マルチコア環境用にスケジューラロジックを修正**
  ...
  マルチコア環境に切り替えるとスケジューラのいくつかの特徴が壊れます。
    ...
    - `start()`内で`switch_to()`を呼び出して見つけたプロセスが初期状態に
      ...
      戻った後にユーザプロセスが使用するジスタを上書きすることは不正となります。
...
この振る舞いを抽象化するシステムリソースです。ユーザプログラムはシステム
...
スタックをゼロから書くのではなく、既存の実装を使用してそれのOSへの統合に焦点を
当てます。具体的には、USPiをイーサネットドライバとして、smoltcpをネットワーク
スタックとして使用します。
...
3. **Ethernet初期化を実装する**
    ...
    **注記: ルータとのセットアップ方法**
    ...
    >   3. ネットワークドライバーに割り当てたIPを手動で割り当てる。
    >
...
4. **ポート管理APIを実装する**
    ...
    フィールドでポートの可用性を管理します。`u64`の整数変数には64ビット
...
> コールとして直接ラップしているためです。そのため、損気コネクションが
